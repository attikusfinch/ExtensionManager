import { beginCell, Cell, Address } from '@ton/ton';

export function createStateInit(codeHex, data) {
    const codeCell = Cell.fromBoc(Buffer.from(codeHex, 'hex'))[0];
    const stateInit = beginCell()
        .storeBit(0)
        .storeBit(0)
        .storeBit(1)
        .storeRef(codeCell)
        .storeBit(1)
        .storeRef(data)
        .storeBit(0)
        .endCell();
    return stateInit;
}

export function createPluginData(walletAddress) {
    const addr = Address.parse(walletAddress);
    return beginCell().storeAddress(addr).endCell();
}

export function createPluginDeployBody() {
    return beginCell().endCell();
}

export function createDeployAndInstallPluginPayload(codeHex, walletAddress, workchain, balance) {
    const pluginData = createPluginData(walletAddress);
    const stateInit = createStateInit(codeHex, pluginData);
    const pluginBody = createPluginDeployBody();
    const payload = beginCell()
        .storeUint(1, 8)
        .storeInt(workchain, 8)
        .storeCoins(balance)
        .storeRef(stateInit)
        .storeRef(pluginBody)
        .endCell();
    return payload.toBoc().toString('base64');
}

export function calculatePluginAddress(stateInit, workchain = 0) {
    const hash = stateInit.hash();
    return new Address(workchain, hash).toString();
}

export function createDeployPluginTransaction(walletAddress, payload, amount) {
    // TON Connect требует amount в наноTON (строка)
    const amountNano = Math.floor(parseFloat(amount) * 1e9).toString();

    return {
        validUntil: Math.floor(Date.now() / 1000) + 600,
        messages: [{
            address: walletAddress,
            amount: amountNano, // в наноTON
            payload: payload
        }]
    };
}

export function deployPredefinedPlugin(walletAddress, deployAmount = '0.1') {
    console.log('🚀 DEPLOY START');
    console.log('Wallet:', walletAddress);

    const CODE_HEX = '';

    const pluginData = createPluginData(walletAddress);
    const stateInit = createStateInit(CODE_HEX, pluginData);
    const pluginAddress = calculatePluginAddress(stateInit, 0);

    console.log('Plugin address:', pluginAddress);

    const balanceNano = BigInt(Math.floor(parseFloat(deployAmount) * 1e9));
    const payload = createDeployAndInstallPluginPayload(CODE_HEX, walletAddress, 0, balanceNano);
    const transaction = createDeployPluginTransaction(walletAddress, payload, deployAmount);

    console.log('✅ READY');

    return { transaction, pluginAddress };
}